...
(function () {
  ...
  Line.prototype.getArc = function () {
    if (typeof this._geodesic === 'undefined') {
      if (this.getP0().isIdeal() || this.getP1().isIdeal()) {
        // TODO this doesn't work if only one point is ideal
        this._calculateGeodesicForIdealPoints();
        ...
        this._calculateGeodesicForPointsOnPlane();
      }
    }
  ...
  Line.prototype._calculateGeodesicForIdealPoints = function () {
  ...
  Line.prototype._calculateGeodesicForPointsOnPlane = function () {
  ...
  Line.hyperbolicIntersect = function (l0, l1) {
    ...
    if (g0IsLine || g1IsLine) {
      ...
      var angleOffset = HyperbolicCanvas.Angle.fromSlope(line.getSlope());
      ...
      var pretendCircle = HyperbolicCanvas.Circle.givenEuclideanCenterRadius(
        HyperbolicCanvas.Point.givenEuclideanPolarCoordinates(
          circle.getEuclideanCenter().getEuclideanRadius(),
          circle.getEuclideanCenter().getAngle() - angleOffset
        ),
      ...
      for (var i = 0; i < pretendIntersects.length; i++) {
        var intersect = HyperbolicCanvas.Point.givenEuclideanPolarCoordinates(
          pretendIntersects[i].getEuclideanRadius(),
          pretendIntersects[i].getAngle() + angleOffset
        );
    ...
    // }
    // return false;
    // TODO check which intersect is on plane?
