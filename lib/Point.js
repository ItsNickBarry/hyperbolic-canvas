...
(function () {
  ...
  */
  ...
  Point.prototype.angle = function () {
    return HyperbolicCanvas.Circle.UNIT.angleAt(this);
  };

  Point.prototype.angleTo = function (otherPoint) {
    var c = HyperbolicCanvas.Line.givenTwoPoints(this, otherPoint).arcCircle();

    if (c) {
      var t = c.tangentAtPoint(this);
      var l = HyperbolicCanvas.Line.givenTwoPoints(otherPoint, c.center);
      var intersect = HyperbolicCanvas.Line.intersect(t, l);
    } else {
      var intersect = otherPoint;
    }

    return Math.atan2(intersect.y - this.y, intersect.x - this.x);
  };
