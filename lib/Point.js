...
(function () {
  ...
  Point.prototype.distantPoint = function (distance, direction) {
    ...
    if (typeof direction === 'undefined') {
      if (typeof this.direction === 'undefined') {
        // use angle of point by default
        angle = this.getAngle();
      } else {
        // use stored direction if direction is not supplied
        angle = this.direction;
      }
    } else {
      // use supplied direction
      angle = direction;
    }
    ...
    if (distance === 0) {
      // TODO implement and use this.clone, and only copy calculated fields
    ...
    // TODO do thsi more efficiently
    distantPoint.direction = distantPoint.angleTo(this) + Math.PI;
    return distantPoint;

    // TODO something else is also wrong; some comets fly astray
  };
  ...
  Point.givenPolarCoordinates = function (euclidianRadius, angle, inverted) {
    if (euclidianRadius < 0) {
      angle += Math.PI;
      euclidianRadius = Math.abs(euclidianRadius);
    }

    return new Point({
      angle: HyperbolicCanvas.Angle.normalize(inverted ? -1 * angle : angle),
      euclideanRadius: euclidianRadius
    });
  };
