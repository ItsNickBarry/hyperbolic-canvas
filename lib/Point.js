...
(function () {
  ...
  Point.prototype.getHyperbolicRadius = function () {
    if (typeof this._hyperbolicRadius === 'undefined') {
      // TODO should be infinity if isIdeal?
  ...
  Point.prototype.euclideanDistantPoint = function (distance, direction) {
    ...
    var distantPoint = Point.givenCoordinates(
  ...
  Point.prototype.hyperbolicAngleTo = function (otherPoint) {
    var c = HyperbolicCanvas.Line.givenTwoPoints(this, otherPoint).getHyperbolicGeodesic();

    if (c instanceof HyperbolicCanvas.Circle) {
      var t0 = c.tangentAtPoint(this);
      var t1 = c.tangentAtPoint(otherPoint);
      var intersect = HyperbolicCanvas.Line.euclideanIntersect(t0, t1);
    } else {
  ...
  Point.prototype.isIdeal = function () {
    // TODO clarify circumstances in which Point or Line is ideal
    return typeof this._hyperbolicRadius === 'undefined' &&
           Math.abs(this.getEuclideanRadius() - 1) < 1e-6;
  };
  ...
  Point.givenEuclideanPolarCoordinates = function (radius, angle, inverted) {
    if (radius < 0) {
      angle += Math.PI;
  ...
  Point.ORIGIN.hyperbolicDistanceTo = function (otherPoint) {
    // TODO Infinity might be handled by otherPoint.getHyperbolicRadius
    // if (otherPoint.isIdeal()) {
    //   return Infinity;
    // }
  ...
  Point.ORIGIN.euclideanDistantPoint = function (distance, direction) {
    // TODO this might change if behaviour of getDirection changes
    return Point.givenEuclideanCenterRadius(distance, direction);
  };

  Point.ORIGIN.hyperbolicDistantPoint = function (distance, direction) {
    // TODO this might change if behaviour of getDirection changes
    return Point.givenHyperbolicPolarCoordinates(distance, direction);
  };
})();
