...
(function () {
  ...
  Canvas.prototype.at = function () {
    ...
    if (coordinates.__proto__ === Point.prototype) {
      ...
      // scale down
      return new Point({ x: coordinates[0] / this.radius - 1, y: coordinates[1] / this.radius - 1, cartesian });
    }
  ...
  Canvas.prototype.fillPolygon = function (vertices) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (!c) {
        ...
        this.ctx.arcTo(control[0], control[1], p1[0], p1[1], c.radius * this.radius * .8);
      }
    }
    this.ctx.fill();
    this.ctx.closePath();
