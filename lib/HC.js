...
(function () {
  ...
  var Canvas = window.HyperbolicCanvas.Canvas = function (el) {
    ...
    // to re-enable:
    // add ResizeSensor.js to lib, load in html, add hyperbolic-canvas-autoresize class, uncomment the following
    // if (this.hasClass('hyperbolic-canvas-autoresize')) {
    //   // TODO unbind this at some point; sensor senses too aggressively and clears canvas
 ...
 // */
 //  Canvas.prototype.pointAt = function (x, y, cartesian) {
 ...
 //      x = x[0];
 //    }
 //    return new Point(x / this.radius - 1, y / this.radius - 1, cartesian);
 //  };
 //
 //  /*
 //  * returns the coordinates of a Point relative to the canvas
 //  * if cartesian == true, returned y axis will not be flipped to match canvas
 //  */
 //  Canvas.prototype.scalePoint = function (point, cartesian) {
 ...
 //  };
  ...
  Canvas.prototype.drawLine = function (l, infinite) {
    ...
    this.ctx.arc(centerScaled[0], centerScaled[1], c.radius * this.radius, 0, TAU);
    this.ctx.stroke();
    this.ctx.closePath();
