...
(function () {
  ...
  Canvas.prototype.drawCircle = function (c, options) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(center[0], center[1], c.getRadius() * this.radius, 0, Math.TAU);
    this.ctx.closePath();
    if (options.fill) {
      this.ctx.fill();
    }
  ...
  Canvas.prototype.strokeLine = function (l, infinite) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(centerScaled[0], centerScaled[1], c.getRadius() * this.radius, a0 < a1 ? a0 : a1, a0 < a1 ? a1 : a0, Math.abs(a0 - a1) > Math.PI);
    this.ctx.stroke();
    this.ctx.closePath();
  ...
  Canvas.prototype.drawPath = function (l) {
    ...
    if (c) {
      ...
      if (control) {
        this.ctx.arcTo(control[0], control[1], p1[0], p1[1], c.getRadius() * this.radius);
      } else {
        this.ctx.lineTo(p1[0], p1[1]);
      }
    } else {
