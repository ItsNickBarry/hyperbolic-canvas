...
(function () {
  ...
  var Canvas = window.HyperbolicCanvas.Canvas = function (el) {
    ...
    this.filledPolygons = []; // TODO store drawn objects on the canvas?
  ...
  Canvas.prototype.strokeLineThroughIdealPoints = function (a0, a1) {
    ...
    var l0 = Line.givenPointSlope(p0, -1 / Angle.slope(a0));
    var l1 = Line.givenPointSlope(p1, -1 / Angle.slope(a1));

    var center = Line.intersect(l0, l1);
    var circle = Circle.givenCenterRadius(center, Line.givenTwoPoints(p0, center).euclideanDistance());
  ...
  Canvas.prototype.strokeRings = function (d, n) {
    for (var i = 0; i < n; i++) {
      var ring = Circle.givenHyperbolicCenterRadius(Point.CENTER, d * i);
