...
(function () {
  ...
  // Canvas.prototype.tesselate = function (order, n, center, radius, rotation) {
  ...
  Canvas.prototype._pathForHyperbolicLineInfinite = function (l, path, options) {
    ...
    if (geodesic instanceof HyperbolicCanvas.Circle) {
      return this._pathForCircle(geodesic, path);
    } else if (geodesic instanceof HyperbolicCanvas.Line) {
      return this._pathForEuclideanLine(geodesic, path, { infinite: true });
    } else {
      return false;
    }
  };

  Canvas.prototype._pathForHyperbolicLine2 = function (l, path, options) {
    // TODO this is a secondary method for pathing Lines
    // advantage: possibly mitigates effects of floating point errors
    // disadvantage: cannot connect edges and fill
    var geodesic = l.getHyperbolicGeodesic();
    ...
    if (geodesic instanceof HyperbolicCanvas.Circle) {
      ...
      var startPoint = this.at(geodesic.euclideanPointAt(startAngle));
      ...
      return this._pathForEuclideanLine(l, path, options);
    } else {
      return false;
    }
  };
  ...
  Canvas.prototype._pathForHyperbolicPolygon = function (p, path, options) {
    ...
    if (options.infinite) {
      for (var i = 0; i < lines.length; i++) {
        this._pathForHyperbolicLineInfinite(lines[i], path, options);
      }
    } else {
      path.moveTo(start[0], start[1]);
