...
(function () {
  ...
  // TODO add multiple paths to queue before stroking
  ...
  Canvas.prototype._drawHyperbolicPolygon = function (p, options) {
    if(p.__proto__ !== HyperbolicCanvas.Polygon.prototype) {
      // TODO this check should be on other methods, as well
      return false;
    }
    var vertices = p.getVertices();
  ...
  Canvas.prototype._setupSize = function () {
    ...
    backdrop.style["border-radius"] = "" + Math.floor(r) + "px";

    canvas.width = canvas.height = d;
    viewport.style["width"] = viewport.style["height"] = "" + d + "px";
  };
})();
