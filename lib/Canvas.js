...
(function () {
  ...
  Canvas.prototype._appendChildren = function () {
  ...
  Canvas.prototype.strokeLineThroughIdealPoints = function (a0, a1) {
    ...
    var circle = HyperbolicCanvas.Circle.givenCenterRadius(
      center,
  ...
  Canvas.prototype.strokeAngles = function (n, rotation) {
    ...
    for (var i = 0; i < n; i++) {
      var idealPoint = this.at(
        HyperbolicCanvas.Point.givenPolarCoordinates(1, angle)
      );
  ...
  Canvas.prototype.drawPolygon = function (p, options) {
    ...
    for (var i = 0; i < n; i++) {
      this.drawPath(HyperbolicCanvas.Line.givenTwoPoints(
        vertices[i],
        vertices[(i + 1) % n]
      ));
    }
  ...
  Canvas.prototype.strokePolygonBoundaries = function (p) {
    ...
    for (var i = 0; i < n; i++) {
      this.strokeLine(HyperbolicCanvas.Line.givenTwoPoints(
        vertices[i],
        vertices[(i + 1) % n]),
        true
      );
