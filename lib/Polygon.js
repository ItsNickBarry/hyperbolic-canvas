...
(function () {
  ...
  Polygon.givenEuclideanNCenterRadius = function (n, center, radius, rotation, inverse) {
    if (n < 3) {
      return false;
    }
    ...
    for (var i = 0; i < n; i++) {
      vertices.push(center.euclideanDistantPoint(radius, direction));
      direction += increment;
  ...
  Polygon.givenHyperbolicNCenterRadius = function (n, center, radius, rotation, inverse) {
    if (n < 3) {
      return false;
    }
    if (!center.isOnPlane()) {
