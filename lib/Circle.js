...
(function () {
  ...
  Circle.prototype.arcLength = function (p0, p1) {
    var d = Line.pointPoint(p0, p1).euclideanDistance();
  ...
  Circle.prototype.pointAt = function (angle) {
    return Point.xY(this.radius * Math.cos(angle) + this.center.x, this.radius * Math.sin(angle) + this.center.y);
  };
  ...
  Circle.prototype.tangentAt = function (p) {
    ...
    return Line.pointSlope(p, Line.pointPoint(this.center, p).perpindicularSlope());
  };
  ...
  Circle.intersects = function (c0, c1) {
    ...
    return [new Point({ x: xi, y: yi }), new Point({ x: xi_prime, y: yi_prime })];
  };

  Circle.centerRadius = function (center, radius) {
    return new Circle({ center: center, radius: radius });
  };

  Circle.hyperbolicCenterRadius = function (center, radius) {
    // TODO make some steps methods on Point class
    // go to center
    // find point radius distance towards center
    // find point radius distance away from center
    // find midpoint of 2 points
    // circle
    var dh = center.distanceFromCenter();
    var angle = Circle.UNIT.angleAt(center);
    var farPoint = Point.hyperbolicRadiusAngle(dh + radius, angle);
    var nearPoint = Point.hyperbolicRadiusAngle(Math.abs(dh - radius), angle + (dh - radius < 0 ? Math.PI : 0));
    return Circle.pointPoint(farPoint, nearPoint);
  };

  Circle.pointPoint = function (p0, p1) {
    var l = Line.pointPoint(p0, p1);
    return new Circle({ center: l.midpoint(), radius: l.euclideanDistance() / 2 })
  };

  Circle.pointPointPoint = function (p0, p1, p2) {
    if (p0.equals(p1) || p0.equals(p2) || p1.equals(p2)) {
      // points are not unique
      return false;
    }
    if (Line.pointPoint(p0, p1).slope === Line.pointPoint(p0, p2).slope) {
      // points are in a straight line
      return false;
    }
    b0 = Line.pointPoint(p0, p1);
    b1 = Line.pointPoint(p1, p2);
    var center = Line.intersect(b0.bisector(), b1.bisector());
    var radius = Line.pointPoint(p0, center).euclideanDistance();
    return new Circle({ center: center, radius: radius });
  };
