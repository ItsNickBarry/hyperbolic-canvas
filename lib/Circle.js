...
(function () {
  ...
  Circle.prototype.equals = function (otherCircle) {
    return this.getCenter().equals(otherCircle.getCenter()) && this.getRadius() === otherCircle.getRadius();
  };
  ...
  Circle.prototype.pointAt = function (angle) {
    return HyperbolicCanvas.Point.givenCoordinates(this.getRadius() * Math.cos(angle) + this.getCenter().getX(), this.getRadius() * Math.sin(angle) + this.getCenter().getY());
  };

  Circle.prototype.yAtX = function (x) {
    var abs = Math.sqrt(Math.pow(this.getRadius(), 2) - Math.pow(x - this.getCenter().getX(), 2));
    return abs ? [this.getCenter().getY() + abs, this.getCenter().getY() - abs] : false;
  };

  Circle.prototype.xAtY = function (y) {
    var abs = Math.sqrt(Math.pow(this.getRadius(), 2) - Math.pow(y - this.getCenter().getY(), 2));
    return abs ? [this.getCenter().getX() + abs, this.getCenter().getX() - abs] : false;
  };

  Circle.prototype.tangentAtAngle = function (a) {
    return HyperbolicCanvas.Line.givenPointSlope(this.pointAt(a), -1 / HyperbolicCanvas.Angle.toSlope(a));
  };
  ...
  Circle.givenHyperbolicCenterRadius = function (center, radius) {
    ...
    var farPoint = HyperbolicCanvas.Point.givenHyperbolicPolarCoordinates(dh + radius, angle);
    var nearPoint = HyperbolicCanvas.Point.givenHyperbolicPolarCoordinates(Math.abs(dh - radius), angle + (dh - radius < 0 ? Math.PI : 0));
  ...
  Circle.givenThreePoints = function (p0, p1, p2) {
    ...
    if (HyperbolicCanvas.Line.givenTwoPoints(p0, p1).getSlope() === HyperbolicCanvas.Line.givenTwoPoints(p0, p2).getSlope()) {
