...
(function () {
  ...
  Circle.prototype.getHyperbolicArea = function () {
    // TODO verify, store result
    // these are approximately equivalent; which is more ill-conditioned?
    return 2 * Math.TAU * Math.pow(Math.sinh(this.getHyperbolicRadius() / 2), 2);
    return Math.TAU * (Math.cosh(this.getHyperbolicRadius()) - 1);
  };
  ...
  Circle.prototype.getHyperbolicCircumference = function (first_argument) {
    // TODO verify, store result
    return Math.TAU * Math.sinh(this.getHyperbolicRadius());
  };
  ...
  Circle.prototype.euclideanPointAt = function (angle) {
    return HyperbolicCanvas.Point.givenCoordinates(
      this.getEuclideanRadius() * Math.cos(angle) + this.getEuclideanCenter().getX(),
      this.getEuclideanRadius() * Math.sin(angle) + this.getEuclideanCenter().getY()
    );
  };

  Circle.prototype.hyperbolicAngleAt = function (p) {
    return this.getHyperbolicCenter().angleTo(p);
  };

  Circle.prototype.hyperbolicPointAt = function (angle) {
    return this.getHyperbolicCenter().distantPoint(this.getHyperbolicRadius(), angle);
  };
  ...
  Circle.prototype._calculateEuclideanCenterRadius = function () {
    ...
    var farPoint = this.hyperbolicPointAt(center.getAngle());
    var nearPoint = this.hyperbolicPointAt(HyperbolicCanvas.Angle.opposite(center.getAngle()));
    var diameter = HyperbolicCanvas.Line.givenTwoPoints(farPoint, nearPoint);
    this._euclideanCenter = diameter.getEuclideanMidpoint();
    this._euclideanRadius = diameter.getEuclideanDistance() / 2;
  };
  ...
  var center = this.getEuclideanCenter();
    if (center.getEuclideanRadius() + this.getEuclideanRadius() >= 1) {
      ...
      // TODO add checks for invalid hyperbolic values on all hyperbolic functions
      this._hyperbolicCenter = this._hyperbolicRadius = NaN;
    }
    var farPoint = this.euclideanPointAt(center.getAngle());
    var nearPoint = this.euclideanPointAt(HyperbolicCanvas.Angle.opposite(center.getAngle()));
    var diameter = HyperbolicCanvas.Line.givenTwoPoints(farPoint, nearPoint);
    this._hyperbolicCenter = diameter.getHyperbolicMidpoint();
    this._hyperbolicRadius = diameter.getHyperbolicDistance() / 2;
  };
  ...
  Circle.givenTwoPoints = function (p0, p1) {
    ...
    return new Circle({
      ...
      euclideanRadius: l.getEuclideanDistance() / 2
    });
  };
  ...
  Circle.UNIT = new Circle({ euclideanCenter: HyperbolicCanvas.Point.ORIGIN, euclideanRadius: 1 });
})();
