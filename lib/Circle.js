...
(function () {
  ...
  Circle.prototype.xAtY = function (y) {
    ...
    if (a) {
      return a < 1e-6 ? [center.getX()] : [center.getX() + a, center.getX() - a];
    } else {
  ...
  Circle.prototype.yAtX = function (x) {
    ...
    if (a) {
      return a < 1e-2 ? [center.getY()] : [center.getY() + a, center.getY() - a];
    } else {
  ...
  Circle.UNIT.hyperbolicPointAt = Circle.UNIT.euclideanPointAt = function (angle) {
