...
(function () {
  ...
  Circle.prototype.arcLength = function (p0, p1) {
    ...
    var d = Line.givenTwoPoints(p0, p1).getEuclideanDistance();
    return Math.acos(1 - (Math.pow(d, 2) / (2 * Math.pow(this.radius, 2))));
  };

  Circle.prototype.angleAt = function (p) {
    var dx = p.getX() - this.center.getX();
    var dy = p.getY() - this.center.getY();
    return Angle.normalize(Math.atan2(dy, dx));
  };

  Circle.prototype.pointAt = function (angle) {
    return Point.givenCoordinates(this.radius * Math.cos(angle) + this.center.getX(), this.radius * Math.sin(angle) + this.center.getY());
  };

  Circle.prototype.yAtX = function (x) {
    var abs = Math.sqrt(Math.pow(this.radius, 2) - Math.pow(x - this.center.getX(), 2));
    return abs ? [this.center.getY() + abs, this.center.getY() - abs] : false;
  };

  Circle.prototype.xAtY = function (y) {
    var abs = Math.sqrt(Math.pow(this.radius, 2) - Math.pow(y - this.center.getY(), 2));
    return abs ? [this.center.getX() + abs, this.center.getX() - abs] : false;
  };

  Circle.prototype.hyperbolicCenter = function () {
    var l = Line.givenTwoPoints(this.center, Point.ORIGIN);
    // var intersects = TODO find intersects between line and circle

  };

  Circle.prototype.tangentAtAngle = function (a) {
    return Line.givenPointSlope(this.pointAt(a), -1 / Angle.toSlope(a));
  };
  ...
  Circle.intersects = function (c0, c1) {
    ...
    var x0 = c0.center.getX();
    var y0 = c0.center.getY();
    var r0 = c0.radius;
    var x1 = c1.center.getX();
    var y1 = c1.center.getY();
    var r1 = c1.radius;
  ...
  Circle.givenTwoPoints = function (p0, p1) {
    ...
    return new Circle({ center: l.getEuclideanMidpoint(), radius: l.getEuclideanDistance() / 2 })
  };
  ...
  Circle.givenThreePoints = function (p0, p1, p2) {
    ...
    if (Line.givenTwoPoints(p0, p1).getSlope() === Line.givenTwoPoints(p0, p2).getSlope()) {
    ...
    var radius = Line.givenTwoPoints(p0, center).getEuclideanDistance();
    return new Circle({ center: center, radius: radius });
  };

  Circle.UNIT = new Circle({ center: Point.ORIGIN, radius: 1 });
})();
